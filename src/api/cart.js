...
export async function addToCart(userId, gameId) {
  try {
    const cartResponse = await fetch(`${API_URL}/cart?user_id=${userId}`);
    if (!cartResponse.ok) {
...
export async function checkoutCart(userId, selectedGameIds) {
  try {
    const boughtResponse = await fetch(`${API_URL}/bought?user_id=${userId}`);
    ...
    if (!bought) {
      ...
      const response = await fetch(`${API_URL}/bought/${bought.id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
